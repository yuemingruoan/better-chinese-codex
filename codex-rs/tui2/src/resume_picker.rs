...
fn render_column_headers(
    ...
    spans.push(tr(language, "对话", "Conversation").bold());
...
mod tests {
    ...
    async fn resume_picker_screen_snapshot() {
...

        {
            ...
            frame.render_widget_ref(
                Line::from(vec![
                    tr(Language::En, "恢复之前的会话", "Resume previous session")
                        .bold()
                        .cyan(),
                ]),
                header,
            ...
            frame.render_widget_ref(
                Line::from(tr(Language::En, "输入以搜索", "Type to search").dim()),
                search,
            );
            ...
            let hint_line: Line = vec![
                key_hint::plain(KeyCode::Enter).into(),
                tr(Language::En, " 恢复会话", " Resume session").dim(),
                "    ".dim(),
                key_hint::plain(KeyCode::Esc).into(),
                tr(Language::En, " 开启新会话", " Start new session").dim(),
                "    ".dim(),
                key_hint::ctrl(KeyCode::Char('c')).into(),
                tr(Language::En, " 退出", " Quit").dim(),
            ]
            .into();
            frame.render_widget_ref(hint_line, hint);
        }
