...
- **逐项提交**：每完成 task.md 列表中的一项（或拆分子项）并通过验证后，立即提交 commit；提交信息写明已完成的功能。  
...
5. **任务执行循环**（对每个任务/子项）：  
   ...
   - **提交变更**：每完成一个任务/子项并验证通过后立刻提交 commit，提交信息以“完成 <功能/子项>”等描述已完成内容。  
   - **勾选进度**：先在 `.codex/task.md` 中勾选完成状态（`[ ]`→`[x]`），可选同步到 Plan/TODO，并在回复中同步结果与测试状态。  
...
汇报节奏（对话中执行）
- **开始前**：报告分支名、将执行的任务 ID/顺序、预期运行的主要命令，并提醒用户开发在独立分支上完成，完成后请再次运行 `/sdd-develop` 选择合并更新。  
