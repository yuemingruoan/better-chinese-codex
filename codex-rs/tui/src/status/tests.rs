...
async fn status_snapshot_includes_reasoning_details() {
    ...
    let rate_display = rate_limit_snapshot_display(&snapshot, captured_at, Language::ZhCn);
...
async fn status_snapshot_shows_unlimited_credits() {
    ...
    assert!(
        rendered
            .iter()
            .any(|line| line.contains("额度") && line.contains("无限额度")),
        "expected 额度: 无限额度 line, got {rendered:?}"
    );
}
